...
.DESCRIPTION
    Lee .env de la raíz del proyecto, establece cada variable como
    environment variable y ejecuta dotnet run. Sin Docker, sin túnel,
    solo localhost.
...
if (-not (Test-Path $envFile)) {
    Write-Host "❌ No se encuentra .env en $ProjectRoot" -ForegroundColor Red
...
Write-Host "   ✅ $count variables cargadas" -ForegroundColor Green

# ─── Resumen de configuración ───────────────────────────────────
Write-Host ""
Write-Host "📋 Configuración:" -ForegroundColor Cyan
Write-Host "   Connection String:    $($env:ConnectionStrings__AgendaDb)" -ForegroundColor Gray
Write-Host "   OpenAI:               $(if ($env:OpenAI__ApiKey -and $env:OpenAI__ApiKey -ne 'sk-xxx') { '✅ configurada' } else { '⚠️  sk-xxx (placeholder)' })" -ForegroundColor $(if ($env:OpenAI__ApiKey -and $env:OpenAI__ApiKey -ne 'sk-xxx') { 'Green' } else { 'Yellow' })
Write-Host "   JWT Secret:           $(if ($env:Jwt__Secret) { '✅ configurado' } else { '❌ faltante' })" -ForegroundColor $(if ($env:Jwt__Secret) { 'Green' } else { 'Red' })
Write-Host "   Master Key:           $(if ($env:TokenEncryption__MasterKey) { '✅ configurada' } else { '❌ faltante' })" -ForegroundColor $(if ($env:TokenEncryption__MasterKey) { 'Green' } else { 'Red' })
...
if ($errors.Count -gt 0) {
    Write-Host "⚠️  Advertencias de configuración:" -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "⚠️  Error en migraciones. ¿SQL Server está corriendo?" -ForegroundColor Yellow
